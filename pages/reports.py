...
def generate_pdf(company_id, overall_score, product_value, recommendations):
    ...
    pdf.cell(200, 10, txt=f"Stickiness Report for Company ID: {company_id}", ln=True, align='C')
    pdf.ln(10)
    pdf.set_font("Arial", size=10)
    pdf.cell(200, 10, txt=f"overall_score: {overall_score}%", ln=True)
    pdf.ln(5)
    pdf.cell(200, 10, txt=f"Product: {product_value}", ln=True)
    pdf.ln(10)
    ...
    for rank, features in recommendations.items():
        if features:
            pdf.cell(200, 10, txt=f"{rank} Stickiness Features:", ln=True)
            pdf.ln(3)
            for feature in features:
                pdf.multi_cell(190, 10, txt=feature, border=0, align='L')
            pdf.ln(5)
...
def reports_page():
    ...
    if "Company ID" in data.columns:
        ...
        # 🟠 **Orange Generate & Download Button**
